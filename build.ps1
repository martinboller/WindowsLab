...
function post_build_checks {
  ...
  $CALDERA_CHECK = download -URL 'https://192.168.10.47:8888' -PatternToMatch '<title>CALDERA</title>'
  Write-Verbose "[post_build_checks] Cladera Result: $CALDERA_CHECK"

  Write-Verbose '[post_build_checks] Running Splunk Check.'
  $SPLUNK_CHECK = download -URL 'https://192.168.10.47:8000/en-US/account/login?return_to=%2Fen-US%2F' -PatternToMatch 'This browser is not supported by Splunk'
  Write-Verbose "[post_build_checks] Splunk Result: $SPLUNK_CHECK"

  Write-Verbose '[post_build_checks] Running Fleet Check.'
  $FLEET_CHECK = download -URL 'https://192.168.10.47:8412' -PatternToMatch 'Kolide Fleet'
