...
post_build_checks() {
  ...
  CALDERA_CHECK=$(curl -ks -m 2 https://192.168.56.5:8888 | grep -c '302: Found' || echo "")
  SPLUNK_CHECK=$(curl -ks -m 2 https://192.168.56.5:8000/en-US/account/login?return_to=%2Fen-US%2F | grep -c 'This browser is not supported by Splunk' || echo "")
  FLEET_CHECK=$(curl -ks -m 2 https://192.168.56.5:8412 | grep -c 'Kolide Fleet' || echo "")
